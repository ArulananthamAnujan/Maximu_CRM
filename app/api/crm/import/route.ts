...
export async function POST(request: Request) {
  try {
    ...
    if(action==="commit"){
      ...
      const rows=await get(`import_rows?select=id,row_number,normalized_data,raw_data,protected_data,source_checksum,status&batch_id=eq.${batchId}&status=eq.valid&order=row_number.asc&limit=50`,token) as Json[];
...
async function importLegacyEntity(entity:string,data:Json,org:string,sourceSystem:string,actor:string,token:string):Promise<string>{
  ...
  const write=async(table:string,value:Json)=>{if(existing){const changes={...value};delete changes.id;await patch(table,id,changes,token);}else await insert(table,value,token);};
  if(entity==="clients"){
    ...
    const caseId=await resolveLegacyReference(org,sourceSystem,"cases",required(data.case_source_key,"Case source key"),"cases","case_number",token);
    const noteBody=required(data.body??data.note??data.remark,"Note");await write("case_notes",{id,organisation_id:org,case_id:caseId,author_id:validUuid(data.resolved_author_id??data.resolved_owner_id??data.author_id)||actor,body:noteBody,visibility:optional(data.visibility)||"case_team",created_at:at(data.created_at??data.created_date??data.date)??undefined});await saveLegacyActivity({org,caseId,sourceEntityType:"notes",sourceKey,eventType:"note",subject:"Legacy note",body:noteBody,actorLabel:optional(data.author??data.created_by??data.added_by),actorProfileId:validUuid(data.resolved_author_id),occurredAt:at(data.created_at??data.created_date??data.date),metadata:{legacy_data:data.legacy_data}},token);
