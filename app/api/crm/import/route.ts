...
async function importLegacyEntity(entity:string,data:Json,org:string,sourceSystem:string,actor:string,token:string):Promise<string>{
  ...
  if(entity==="clients"){
    ...
    await insert("cases",{id,organisation_id:org,client_id:clientId,branch_id:data.branch_id,case_number:data.case_number||sourceKey,service_type:data.service_type||"study_abroad",matter_type:optional(data.matter_type??data.visa_type??data.application_type),owner_id:validUuid(data.resolved_owner_id??data.owner_id),health:String(data.health??"").toLowerCase()==="critical"?"critical":"healthy",priority:optional(data.priority)||"medium",progress:importedProgress===null?lifecycleProgress(lifecycle):Math.min(100,Math.max(0,numberValue(importedProgress))),target:optional(data.target??data.country),due_at:dateValue(data.due_at??data.due_date),lifecycle_stage:lifecycle,visa_expiry_on:dayValue(data.visa_expiry_on??data.visa_expiry_date??data.current_visa_expiry??data.date_of_expiry),custom_fields:{legacy_source_key:sourceKey,legacy_data:data.legacy_data}},token);
