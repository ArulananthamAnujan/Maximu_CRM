...
async function importLegacyEntity(entity:string,data:Json,org:string,sourceSystem:string,actor:string,token:string):Promise<string>{
  ...
  if(entity==="clients"){
    await insert("clients",{id,organisation_id:org,branch_id:data.branch_id,crm_id:data.crm_id||`LEGACY-${id.slice(0,8).toUpperCase()}`,first_name:data.first_name,last_name:data.last_name,email:data.email,mobile:data.mobile,date_of_birth:dayValue(data.date_of_birth),nationality:data.nationality,gender:data.gender,marital_status:optional(data.marital_status)?.toLowerCase(),country_of_birth:data.country_of_birth,current_country:data.current_country,preferred_language:data.preferred_language,passport_country:data.passport_country,passport_expiry:dayValue(data.passport_expiry),address:{line1:data.address_line??null,city:data.city??null,state:data.state??null,postcode:data.postcode??null},source:"legacy_import",current_lifecycle:data.current_lifecycle||"enquiry",...(data.passport_number_encrypted?{passport_number_encrypted:data.passport_number_encrypted,passport_masked:data.passport_masked}:{}),custom_fields:{legacy_source_key:sourceKey,alternatePhone:data.alternate_phone??null,legacy_data:data.legacy_data},updated_at:new Date().toISOString()},token);
    ...
    await insert("cases",{id,organisation_id:org,client_id:clientId,branch_id:data.branch_id,case_number:data.case_number||sourceKey,service_type:data.service_type||"study_abroad",matter_type:optional(data.matter_type??data.visa_type??data.application_type),owner_id:validUuid(data.resolved_owner_id??data.owner_id),health:String(data.health??"").toLowerCase()==="critical"?"critical":"healthy",priority:optional(data.priority)||"medium",progress:importedProgress===null?lifecycleProgress(lifecycle):Math.min(100,Math.max(0,numberValue(importedProgress))),target:optional(data.target??data.country),next_action:optional(data.sub_status??data.enquiry_status??data.status),due_at:dateValue(data.due_at??data.due_date),lifecycle_stage:lifecycle,visa_expiry_on:dayValue(data.visa_expiry_on??data.visa_expiry_date??data.current_visa_expiry??data.date_of_expiry),custom_fields:{legacy_source_key:sourceKey,mainStatus:optional(data.main_status),subStatus:optional(data.sub_status??data.enquiry_status),visaType:optional(data.visa_type),legacy_data:data.legacy_data}},token);
    ...
    const caseId=await resolveLegacyReference(org,sourceSystem,"cases",required(data.case_source_key,"Case source key"),"cases","case_number",token);
    const completedAt=dateValue(data.completed_at??data.completed_date);
    await insert("tasks",{id,organisation_id:org,case_id:caseId,title:required(data.title??data.task,"Task title"),description:optional(data.description??data.details),task_type:optional(data.task_type??data.type)||"case_work",assigned_to:validUuid(data.resolved_owner_id??data.assigned_to),assigned_by:actor,priority:optional(data.priority)||"medium",status:optional(data.status)||"open",due_at:dateValue(data.due_at??data.due_date),completed_at:completedAt,completed_by:completedAt?validUuid(data.completed_by)||actor:null,updated_at:dateValue(data.updated_at)??completedAt??new Date().toISOString()},token);
...
async function importLegacyCombined(entity:string,data:Json,org:string,sourceSystem:string,actor:string,token:string):Promise<string>{
  ...
  const educationInstitution=optional(data.education_institution??data.college??data.board??data.school);
  const qualification=optional(data.qualification??data.highest_qualification??data.education_level);
  if(educationInstitution||qualification)await insert("client_education_history",{id:crypto.randomUUID(),organisation_id:org,client_id:clientId,country_code:optional(data.education_country??data.study_country),institution:educationInstitution||"Not supplied",qualification:qualification||"Not supplied",field_of_study:optional(data.subjects??data.subject_stream??data.field_of_study??data.stream),started_on:dayValue(data.education_start??data.started_on),completed_on:dayValue(data.education_end??data.year_passed??data.year_of_passing),result:optional(data.percentage??data.grade??data.result),details:{backlogs:numberValue(data.backlogs,0),legacy_import:true}},token);
  const employer=optional(data.company_name??data.employer??data.organization);
  const position=optional(data.position??data.job_title??data.designation);
  if(employer||position)await insert("client_employment_history",{id:crypto.randomUUID(),organisation_id:org,client_id:clientId,employer:employer||"Not supplied",job_title:position||"Not supplied",country_code:optional(data.employment_country??data.work_country),started_on:dayValue(data.work_start_date??data.employment_start??data.started_on),ended_on:dayValue(data.work_end_date??data.employment_end??data.ended_on),hours_per_week:numberValue(data.hours_per_week,0)||null,duties:optional(data.duties??data.main_duties),details:{total_experience:optional(data.total_experience??data.experience),legacy_import:true}},token);
  const testType=optional(data.english_test??data.test_type??data.proficiency_test);
  if(testType)await insert("english_tests",{id:crypto.randomUUID(),organisation_id:org,client_id:clientId,test_type:testType,test_date:dayValue(data.test_date??data.exam_date),overall:numberValue(data.overall??data.overall_score,0)||null,listening:numberValue(data.listening??data.listening_score,0)||null,reading:numberValue(data.reading??data.reading_score,0)||null,writing:numberValue(data.writing??data.writing_score,0)||null,speaking:numberValue(data.speaking??data.speaking_score,0)||null,details:{legacy_import:true}},token);
  if(entity==="study_records"&&(optional(data.interested_country??data.destination_country??data.country)||optional(data.university??data.institution??data.course)))await insert("study_preferences",{id:crypto.randomUUID(),organisation_id:org,client_id:clientId,destination_countries:[optional(data.interested_country??data.destination_country??data.country)].filter(Boolean),study_levels:[optional(data.apply_level??data.study_level??data.qualification)].filter(Boolean),preferred_institutions:[optional(data.university??data.institution)].filter(Boolean),fields_of_study:[optional(data.course??data.interested_course)].filter(Boolean),target_intakes:[optional(data.intake??data.suggested_intake)].filter(Boolean),annual_budget:numberValue(data.annual_budget??data.budget,0)||null,funding_source:optional(data.funding_source),accommodation_required:/yes|true|1/i.test(String(data.accommodation_required??"")),scholarship_required:/yes|true|1/i.test(String(data.scholarship_required??"")),notes:optional(data.preference_notes??data.remarks)},token);
