...
export function MobileNavigation({ active, client, navigate, menu }: {
  ...
  return <nav className="mobileTabbar" aria-label="Mobile navigation">
    {items.filter(item=>allowed(item.key)).map(({ key, label, Icon }) => <button key={key} aria-current={active === key ? "page" : undefined} onClick={() => navigate(key)}><Icon size={22} /><span>{label}</span></button>)}
