...
export function StaffDeleteDialog({ person, profiles, onDelete, onClose }: {
  ...
  return <div className="modalBackdrop staffDeleteBackdrop" ref={panel} role="dialog" aria-modal="true" aria-label="Delete staff account" tabIndex={-1}>
    <section className="staffDeleteDialog">
      <header><div><span className="kicker">STAFF ACCOUNT</span><h2>Delete {person.display_name}?</h2></div><button type="button" className="ghostButton" disabled={busy} onClick={close}>Cancel</button></header>
      <p className="staffDeleteIdentity">{person.email}</p>
      <p>This permanently removes their login and takes them out of the team list. You can then create a fresh account with the same email.</p>
      ...
      <label htmlFor={`${fieldId}-replacement`}>Hand over open responsibilities</label>
